...
export async function GET(request: Request, context: RouteContext) {
  ...
  if (!accessToken) {
    accessToken = (cookieStore.get(`${GUEST_ORDER_TOKEN_COOKIE_PREFIX}${orderId}`)?.value || "").trim();
  }
  ...
  const response = await fetchMetrikApi(`/web/payments/orders/${orderId}/status${paymentQuery}`, {
    headers: {
